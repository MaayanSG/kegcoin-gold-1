...
#define CACHE_ROUNDS    2                  // number of rounds in cache production
...
// Assuming 1 block per 30 secconds, an epoch estimates 30 minutes
...
void keghash_0(uint8_t* data, uint8_t* out){
	uint8_t   shift[4]   = {0};
	uint64_t  key[2][2]  = {0};
	uint64_t  crc_64[4]  = {0};
...
// keghash_1 uses a lookup in addition to the previous operations
// to force a hasher to have 256B read-only scratchpad in L1 cache
void keghash_1(uint8_t* data, uint8_t* scratchpad, uint8_t* out){
	uint8_t   shift[4]   = {0};
	uint64_t  key[2][2]  = {0};
	uint64_t  divr[2]    = {0};
	uint64_t  crc_64[4]  = {0};
...
// Difference from keghash_1 is a 16bit integer is used to obtain the
// data from the scratchpad. (64KiB scratchpad)
void keghash_2(uint8_t* data, uint8_t* scratchpad, uint8_t* out){
	uint8_t   shift[4]   = {0};
	uint64_t  key[2][2]  = {0};
	uint64_t  divr[2]    = {0};
	uint64_t  crc_64[4]  = {0};
...
// Difference from keghash_2 is a 32bit integer is used to obtain the
// data from the dataset. (4 GiB dataset)
void keghash_3_full(uint8_t* data, uint64_t* dataset, uint8_t* out){
	uint8_t   shift[4]      = {0};
	uint64_t  key[2][2]     = {0};
	uint64_t  divr[2]       = {0};
	uint64_t  crc_64[4]     = {0};
...
void keghash_3_light(uint8_t* data, uint8_t* cache, uint8_t* out){
	uint8_t   shift[4]        = {0};
	uint64_t  key[2][2]       = {0};
	uint64_t  divr[2]         = {0};
	uint64_t  crc_64[4]       = {0};
...
void make_scratchpad(uint8_t* seed, uint8_t* scratchpad){
	keghash_0(seed, scratchpad);
	for(uint32_t i=32; i<65536;i+=32) keghash_0(&scratchpad[i-32], &scratchpad[i]);
}
...
void keghash_pow_light(uint8_t* data, uint32_t lenght, uint8_t* cache, uint8_t* result){
	...
	uint8_t*  seed       = (uint8_t*)seed_64; 
	blake2b(seed, 32, data, (size_t) lenght, NULL, 0);
	keghash_3_light(seed, cache, result);
}

void get_seedhash(uint32_t block_number, uint8_t* seed){ /* IN: block number | OUT: seed */
	for(uint32_t i=0;i<block_number/EPOCH_LENGTH;i++) keghash_0(seed, seed);
}
...
void keghash_light_api(const void* data, uint32_t length, uint8_t* hash_out, uint32_t height){
	uint64_t* cache_64    = (uint64_t*)calloc(8388608,8);
	uint8_t*  cache       = (uint8_t*)cache_64;
	if (!cache_64) error_exit(1);
	cache_from_height(height, cache);
